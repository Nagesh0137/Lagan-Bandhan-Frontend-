...
import { isProfileMatch } from '../utils/casteReligionMatcher';
  ...
  export default function HomeScreen() {
    ...
    const {data: profileData,isLoading: isProfileListLoading,isFetching,} = useGetProfilesQuery(
      {
        fullName: fullName,
        selectedAge,
        selectedIncomeRange,
    ...
    if (!data?.redirectTo) {
      ...
      const religion = `${data?.data?.userDetails?.personalDetails?.religion}`;
      const caste = `${data?.data?.userDetails?.personalDetails?.caste}`;
      ...
      localStorage.setItem('caste', caste);
    ...
    const setData = async (item: any) => {
      ...
      if (data) {
        ...
        if (isAlreadyAdded.length >= 1) {
          showToast(t('homeScreen.toast.alreadyAddedFavorites'), 'error');
        } else {
          newArray = JSON.parse(data);
          newArray.push(item);
          localStorage.setItem(LocalStorageKey, JSON.stringify(newArray));
          showToast(t('homeScreen.toast.addedToFavorites'), 'success');
        }
      } else {
        newArray.push(item);
        localStorage.setItem(LocalStorageKey, JSON.stringify(newArray));
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-red-50">
          ...
          <div className="relative z-10 px-4 sm:px-6 lg:px-8 py-12 lg:py-20">
            <div className="max-w-7xl mx-auto text-center">
              <h1 className="text-3xl md:text-4xl lg:text-6xl font-bold text-white mb-4 font-Onest">
                {t('homeScreen.hero.findYourPerfect')}
                <span className="block text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 to-orange-300">
                  {t('homeScreen.hero.lifePartner')}
                </span>
              </h1>
                ...
                <div className="bg-white/95 backdrop-blur-sm rounded-2xl p-6 shadow-2xl">
                  <div className="flex flex-col lg:flex-row gap-4 items-center">
                    ...
                    <div className="relative flex-1 w-full">
                      ...
                      <input
                        ...
                        placeholder={t('homeScreen.hero.searchPlaceholder')}
                    ...
                    <div className="flex gap-3">
                      ...
                      <button
                        ...
                        {t('homeScreen.hero.searchButton')}
                      </button>
                    </div>
                  </div>
                  ...
                  {(selectedAge || selectedIncomeRange || fullName) && (
                    <div className="mt-4 flex flex-wrap gap-2">
                      <span className="text-sm text-gray-600 font-medium font-Onest">{t('homeScreen.filters.activeFilters')}</span>
                      {fullName && (
                        <span className="inline-flex items-center px-3 py-1 rounded-full text-sm bg-red-100 text-red-800 font-Onest">
                          {t('homeScreen.filters.name')}: {fullName}
                          <button
                            type="button"
                      ...
                      {selectedAge && (
                        <span className="inline-flex items-center px-3 py-1 rounded-full text-sm bg-blue-100 text-blue-800 font-Onest">
                          {t('homeScreen.filters.age')}: {selectedAge}
                          <button
                            type="button"
                      ...
                      {selectedIncomeRange && (
                        <span className="inline-flex items-center px-3 py-1 rounded-full text-sm bg-green-100 text-green-800 font-Onest">
                          {t('homeScreen.filters.income')}: {selectedIncomeRange}
                          <button
                            type="button"
        ...
        <div className="relative z-10 -mt-8">        {isLoading || isProfileListLoading || isFetching ? (
          <div className="min-h-[400px] flex items-center justify-center">
            <Spinner />
          </div>
        ) : (() => {
          // Calculate filtered profiles
          const userReligion = localStorage.getItem('religion') || '';
          const userCaste = localStorage.getItem('caste') || '';
          const filteredProfiles = profileData?.data?.userData?.filter((item: any) => {
            const profileReligion = item?.userDetails?.personalDetails?.religion || '';
            const profileCaste = item?.userDetails?.personalDetails?.caste || '';
            return isProfileMatch(userReligion, userCaste, profileReligion, profileCaste);
          }) || [];
            ...
            <div className="min-h-[400px] flex items-center justify-center">
              <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md mx-4">
                <NotFound
                  ...
                  label={t('homeScreen.results.noMatchingProfiles')}
                />
                <p className="text-gray-600 text-center mt-4 font-Onest">
                  {t('homeScreen.results.noMatchingDescription')}
                </p>
              </div>
            </div>
          ) : (
            <>
                  ...
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                      <h2 className="text-2xl lg:text-3xl font-bold text-gray-900 font-Onest">
                        {t('homeScreen.results.discoverProfiles')}
                      </h2>
                      <p className="text-gray-600 mt-1 font-Onest">
                        {(() => {
                          ...
                          const userReligion = localStorage.getItem('religion') || '';
                          const userCaste = localStorage.getItem('caste') || '';
                          const filteredCount = profileData?.data?.userData?.filter((item: any) => {
                            const profileReligion = item?.userDetails?.personalDetails?.religion || '';
                            const profileCaste = item?.userDetails?.personalDetails?.caste || '';
                            return isProfileMatch(userReligion, userCaste, profileReligion, profileCaste);
                          })?.length || 0;
                          ...
                          return filteredCount > 0 
                            ? `${filteredCount} ${t('homeScreen.results.matchingProfilesFound')}` 
                            : t('homeScreen.results.browseVerifiedProfiles');
                        })()}
                      </p>
                    </div>
              ...
              <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-12">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 lg:gap-8">
                    {profileData?.data?.userData?.filter((item: any) => {
                      // Filter profiles using intelligent matching for religion and caste variations
                      const userReligion = localStorage.getItem('religion') || '';
                      const userCaste = localStorage.getItem('caste') || '';
                      const profileReligion = item?.userDetails?.personalDetails?.religion || '';
                      const profileCaste = item?.userDetails?.personalDetails?.caste || '';

                      // Use the intelligent matching function that handles variations
                      return isProfileMatch(userReligion, userCaste, profileReligion, profileCaste);
              ...
              {profileData?.data?.count % 10 === 0 && (
                <div className="text-center mb-12">
                  <button
                    ...
                    {t('homeScreen.results.loadMoreProfiles')}
                  </button>
                </div>
              )}
            </>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="text-center mb-12">
                <h3 className="text-3xl lg:text-4xl font-bold text-white mb-4 font-Onest">
                  {t('homeScreen.statistics.trustedByThousands')}
                </h3>
                <p className="text-gray-300 text-lg font-Onest">
                  {t('homeScreen.statistics.growingCommunity')}
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
                <div className="text-center">
                  ...
                  <div className="text-gray-300 font-Onest">{t('homeScreen.statistics.activeProfiles')}</div>
                </div>
                <div className="text-center">
                  <div className="text-4xl lg:text-5xl font-bold text-rose-400 mb-2 font-Onest">5K+</div>
                  <div className="text-gray-300 font-Onest">{t('homeScreen.statistics.successStories')}</div>
                </div>
                <div className="text-center">
                  <div className="text-4xl lg:text-5xl font-bold text-pink-400 mb-2 font-Onest">99%</div>
                  <div className="text-gray-300 font-Onest">{t('homeScreen.statistics.verifiedProfiles')}</div>
                </div>
                <div className="text-center">
                  <div className="text-4xl lg:text-5xl font-bold text-orange-400 mb-2 font-Onest">24/7</div>
                  <div className="text-gray-300 font-Onest">{t('homeScreen.statistics.support')}</div>
                </div>
              </div>
            </div>
          </div>
